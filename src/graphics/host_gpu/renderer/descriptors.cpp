...
static bool IsSupportedStorageTextureDescriptor(const ShaderRecompiler::IR::ImageResource& resource,
	...
	// image, so the 4KB layout would only matter if the contents were published back to guest memory
	// (no linear->4KB retile exists, so such a readback still aborts cleanly). This holds for 2D and
	// 3D alike -- the guest tiling is bypassed either way. A *readable* 4KB storage image, whose
	// guest tiling would have to be interpreted on upload, stays rejected.
